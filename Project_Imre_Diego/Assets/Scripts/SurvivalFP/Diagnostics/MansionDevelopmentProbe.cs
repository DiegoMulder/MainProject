...
namespace SurvivalFP
{
    ...
    public sealed class MansionDevelopmentProbe : MonoBehaviour
    {
        ...
        [Serializable] public class Snapshot
        {
            ...
            public bool paused,light;
            public Vector3 position;
            public string[] inventory,items,doors;
        }
        string directory,mode,cloudCode,lastError=""; int handled; float nextSnapshot; bool drive,cloud;
        ...
        void Update()
        {
            ...
            if(drive && player) player.SubmitMovement(new PlayerCommand {Move=Vector2.up,Sprint=true},player.View);
                    ...
                    try
                    {
                        switch(command.action)
                        {
                            ...
                            case "interact":
                                ...
                                { Vector3 point=target.transform.position+(target.GetComponent<DoorInteractable>()?Vector3.up*1.3f:target.GetComponent<DownedInteractable>()?Vector3.up*.45f:Vector3.zero);player.View.rotation=Quaternion.LookRotation(point-player.View.position);player.Interaction.Tick(player.View,true); }
                                break;
                            ...
                            case "drive":player.Controller.enabled=false;drive=true;break;
