...
namespace SurvivalFP
{
    public class DoorInteractable : NetworkBehaviour, IInteractable
    {
        ...
        public virtual void Interact(PlayerInteraction player) { if(IsServer && interactable) SetOpen(!Open.Value); }
        public void SetOpen(bool value)
        {
            if(!IsServer || Open.Value==value) return;
            Open.Value=value; GameplayNoiseSystem.Emit(transform.position,noiseRadius,NoiseCategory.Door,gameObject);
        }
