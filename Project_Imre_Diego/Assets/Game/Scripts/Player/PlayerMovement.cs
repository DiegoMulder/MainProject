...
namespace SurvivalFP
{
    ...
    public sealed class PlayerMovement : MonoBehaviour
    {
        ...
        float crawlSpeed=.65f;
        const float ProneHeight=.65f;
        public event Action Teleported;
        public void SetDowned(bool downed,float speed)
        {
            Downed=downed;crawlSpeed=Mathf.Max(0,speed);
            horizontalVelocity=slideVelocity=Vector3.zero;verticalSpeed=0;
            bufferRemaining=coyoteRemaining=heightVelocity=0;
            if(downed){capsule.height=ProneHeight;capsule.center=Vector3.up*ProneHeight*.5f;IsCrouching=true;}
        }
        public MotorSnapshot Capture()=>new MotorSnapshot {
            position=transform.position,horizontal=horizontalVelocity,slide=slideVelocity,normal=groundNormal,
            vertical=verticalSpeed,height=capsule.height,heightVelocity=heightVelocity,coyote=coyoteRemaining,
            buffer=bufferRemaining,airLimit=airSpeedLimit,actualSpeed=ActualSpeed,state=State,
            grounded=IsGrounded,sliding=IsSliding,crouching=IsCrouching,ceiling=CeilingBlocked,
            stamina=stamina.Capture()
        };
        public void Restore(MotorSnapshot state)
        {
            bool active=capsule.enabled;capsule.enabled=false;transform.position=state.position;capsule.enabled=active;
            horizontalVelocity=state.horizontal;slideVelocity=state.slide;groundNormal=state.normal;
            verticalSpeed=state.vertical;capsule.height=state.height;capsule.center=Vector3.up*state.height*.5f;
            heightVelocity=state.heightVelocity;coyoteRemaining=state.coyote;bufferRemaining=state.buffer;
            airSpeedLimit=state.airLimit;ActualSpeed=state.actualSpeed;State=state.state;
            IsGrounded=state.grounded;IsSliding=state.sliding;IsCrouching=state.crouching;CeilingBlocked=state.ceiling;
            stamina.Restore(state.stamina);Physics.SyncTransforms();
        }
        ...
        public void ApplyNetworkPresentation(MovementState state, float height, Vector3 velocity, bool grounded, float dt)
        {
            ...
            if (landed) if(!Replaying)Landed?.Invoke(Mathf.Abs(velocity.y));
            if(!Replaying)Simulated?.Invoke(dt);
        }

        public void Tick(PlayerCommand command, float dt)
        {
            if (dt <= 0f) return;
            if(Downed){command.Sprint=false;command.JumpPressed=false;command.Crouch=true;}
            ...
            float speed = Downed?crawlSpeed:SpeedFor(nextState);
            ...
            if (!wasSupported && (IsGrounded || IsSliding) && impactVelocity < -groundStickSpeed - 0.5f)
                if(!Replaying)Landed?.Invoke(-impactVelocity);
        ...
        void SetState(MovementState state)
        {
            ...
            State = state;
            if(!Replaying)StateChanged?.Invoke(state);
        }
