...
namespace SurvivalFP
{
    ...
    public sealed class PlayerRadio : NetworkBehaviour
    {
        ...
        NetworkPlayer player;float nextReport,nextNoise,lastReport;
        ...
        public bool WantsTransmit=>Eligible && !player.Paused && !LocalSettings.Muted && player.Controller.RadioHeld;
        ...
        public void Report(bool pressed,bool speaking)
        {
            if(!IsOwner || Time.unscaledTime<nextReport)return;
            nextReport=Time.unscaledTime+.1f;ReportRpc(pressed,speaking);
        }
        [Rpc(SendTo.Server,InvokePermission=RpcInvokePermission.Owner)]
        void ReportRpc(bool pressed,bool speaking)
        {
            lastReport=Time.unscaledTime;
            Transmitting.Value=pressed && Eligible;
