...
namespace SurvivalFP.Editor
{
    ...
    public static class InventorySetup
    {
        ...
        const string PickupPrefabPath = "Assets/SurvivalFP/Prefabs/PickupItem.prefab";
        const string PickupablesFolder = "Assets/SurvivalFP/Pickupables";
        ...
        static void MakePickup(string name, PickupKind kind, Vector3 position, Color color, PrimitiveType primitive, float scale)
        {
            ...
            if (!beam)
            {
                var template = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Resources/FlashlightBeam.prefab");
                if (template)
                {
            ...
            var clip = AssetDatabase.LoadAssetAtPath<AudioClip>("Assets/SurvivalFP/Audio/switch_002.ogg");
        ...
        {
            var existing = AssetDatabase.LoadAssetAtPath<Material>("Assets/SurvivalFP/Materials/" + name + ".mat");
            if (existing) return existing;
            var mat = new Material(Shader.Find("Universal Render Pipeline/Lit")) { name = name };
            mat.SetColor("_BaseColor", color);
            AssetDatabase.CreateAsset(mat, "Assets/SurvivalFP/Materials/" + name + ".mat");
            return mat;
        }
        ...
        static void EnsurePickupablePrefabs()
        {
            if (!AssetDatabase.IsValidFolder(PickupablesFolder))
                AssetDatabase.CreateFolder("Assets/SurvivalFP", "Pickupables");
