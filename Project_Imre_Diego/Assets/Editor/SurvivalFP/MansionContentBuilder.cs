...
namespace SurvivalFP.Editor
{
    ...
    {
        public const string Root="Assets/SurvivalFP/Mansion";
        ...
        public static void Build()
        {
            ...
            if(File.Exists(Root+"/MansionSettings.asset")) throw new InvalidOperationException("Mansion content already exists. Edit its prefabs/settings rather than overwriting them.");
            ...
            foreach(var folder in new[]{"Materials","Rooms","Props","Prefabs"}) Directory.CreateDirectory(Root+"/"+folder);
            ...
            var player=Player(); var torch=Pickup("Flashlight", "Assets/SurvivalFP/Pickupables/Flashlight.prefab",false);
            ...
            AssetDatabase.CreateAsset(settings,Root+"/MansionSettings.asset");
            ...
            var round=Save(roundGO,"Prefabs/Mansion Round").GetComponent<RoundManager>();
            ...
            AssetDatabase.CreateAsset(list,Root+"/NetworkPrefabs.asset"); manager.NetworkConfig.Prefabs.NetworkPrefabsLists.Add(list);
            ...
            RenderSettings.skybox=AssetDatabase.LoadAssetAtPath<Material>("Assets/SurvivalFP/Materials/URP Sky.mat");
            EditorSceneManager.SaveScene(scene,"Assets/Scenes/MainMenu.unity");
            EditorBuildSettings.scenes=new[]{new EditorBuildSettingsScene("Assets/Scenes/MainMenu.unity",true)}.Concat(EditorBuildSettings.scenes.Where(s=>s.path!="Assets/Scenes/MainMenu.unity")).ToArray();
        ...
        {var mat=new Material(Shader.Find("Universal Render Pipeline/Lit")){name=name};mat.SetColor("_BaseColor",color);mat.SetFloat("_Metallic",metallic);mat.SetFloat("_Smoothness",.3f);AssetDatabase.CreateAsset(mat,Root+"/Materials/"+name+".mat");return mat;}
        ...
        static RoomModule Room(string name,float width,float length,int[] sides,GameObject table,GameObject shelf,GameObject plant,GameObject cabinet)
        {
            ...
            return Save(root,"Rooms/"+name).GetComponent<RoomModule>();
        }
        static NetworkPlayer Player()
        {
            var root=(GameObject)PrefabUtility.InstantiatePrefab(AssetDatabase.LoadAssetAtPath<GameObject>("Assets/SurvivalFP/Prefabs/SurvivalPlayer.prefab"));
            ...
            return Save(root,"Prefabs/Network Survivor").GetComponent<NetworkPlayer>();
        }
        static NetworkPickup Pickup(string name,string source,bool objective)
        {
            ...
            return Save(root,"Prefabs/"+name).GetComponent<NetworkPickup>();
        }
        static DoorInteractable Door(bool exit)
        {
            ...
            door.audioSource=root.AddComponent<AudioSource>();door.audioSource.spatialBlend=1;door.audioSource.playOnAwake=false;door.audioSource.volume=.25f;door.sound=AssetDatabase.LoadAssetAtPath<AudioClip>("Assets/SurvivalFP/Audio/switch_002.ogg");
            return Save(root,"Prefabs/"+root.name).GetComponent<DoorInteractable>();
        }
        static EnemyController Enemy()
        {
            ...
            return Save(root,"Prefabs/Mansion Stalker").GetComponent<EnemyController>();
        }
    }
}
