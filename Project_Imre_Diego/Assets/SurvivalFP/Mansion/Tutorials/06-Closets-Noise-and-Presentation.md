...
Tables now behave as ordinary furniture. Their tabletop/leg collision and crouching clearance remain, but there is no table hiding state or special table detection rule.

## Recipe: add a different wardrobe model

1. Stop Play Mode and duplicate **Assets/SurvivalFP/Mansion/Props/Closet.prefab**.
...
6. Save, register the new prefab in Mansion/NetworkPrefabs.asset, then add it to a closet anchor's Variants list.
...
| Dropped-item collision | Pickup prefab > ImpactNoiseEmitter: Minimum Speed, Cooldown, Radius Per Speed, Maximum Radius |
...
Drops emit their main noise when the physical object hits something, not when Q is pressed. Slow contacts below Minimum Speed are ignored; stronger contacts scale up to Maximum Radius; Cooldown prevents bounce spam. Impact Clip and Maximum Volume configure audible impact playback independently of the AI radius. The server evaluates the actual collision and sends the audio event to peers.
...
Open **Mansion/Prefabs/Network Survivor.prefab** and find **HeartbeatFeedback**.
...
Select **Assets/SurvivalFP/Mansion/PsxVisuals.asset**.
